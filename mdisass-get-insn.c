...
list_head * get_insn(disass_data *data, list_head *insn_list)
{
    ...
    // only for mdisass-modrm project
    if ((data->code->b < 0x00) || (data->code->b > 0x03)) {
        printf("WARNING: It's mdisass-modrm project, %.2X is not an opcode implemented, there are only: 0x00, 0x01, 0x02 and 0x03 with prefixes 0x66 and 0x67\n", data->code->b);
        fetch_data(data, sizeof(uint8_t));
        return NULL;
    }
